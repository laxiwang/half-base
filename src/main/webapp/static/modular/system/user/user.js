...
MgrUser.initColumn = function () {
    var columns = [
        ...
        {title: '分会', field: 'deptName', align: 'center', valign: 'middle', sortable: true},
 ...
 * @param account
 */
MgrUser.runAs = function () {
    if (this.check()) {
        var account = this.seItem.account;
        if(account=='admin'){
        	 Feng.error("不可切换至管理员账号!");
        	return false;
        }
        var ajax = new $ax(Feng.ctxPath + "/runAs", function (data) {
    		window.parent.location.reload();
        }, function (data) {
            Feng.error("切换失失败! 服务器异常!");
        });
 ...
 *退出代理
 */
MgrUser.runRelease = function () {
    if (this.check()) {
        window.open(Feng.ctxPath + "/runRelease");
     }
}
...
MgrUser.search = function () {
    ...
    queryData['deptid'] = MgrUser.branchsalerid;
...
MgrUser.onClickDept = function (e, treeId, treeNode) {
    MgrUser.branchsalerid = treeNode.id;
    MgrUser.search();
};
